...
export default {
  ...
  setup() {
    ...
    const getPreGenerateInfo = async () => {
      ...
      return genResult.map(({ fileContent, fileName, path, fileType }) => {
    ...
    const generate = async () => {
      ...
      try {
        // 暂存待生成代码文件信息
        const saveFilesInfo = await getPreGenerateInfo()

        const filesMap = {}

        saveFilesInfo.forEach((item) => {
          filesMap[item.filePath] = { content: item.fileContent, isBinary: false }
        })

        // eslint-disable-next-line
        const parameters = getParameters({ files: filesMap })
        ...
        document.body.innerHTML = `
          <iframe src="https://codesandbox.io/embed/basic-structure-ant-design-vue-4-2-3-dr44kp?fontsize=14&hidenavigation=1&theme=dark"
             style="width:100%; height:500px; border:0; border-radius: 4px; overflow:hidden;"
             title="Basic Structure - ant-design-vue@4.2.3"
